...
def ai_coach(req: AICoachRequest):
    ...
    try:
        ...
        for act in engine.action_history:
            ...
            context_str += f"[{act['street']}] {act['actor']}: {act['action']}{amt_str} (勝率目安: {round(act['equity']*100, 1)}%)\n"
            
        system_prompt = f"""あなたはGTO（ゲーム理論最適化）理論に基づくプロのポーカーコーチです。
ユーザーから提供される「現在進行中、または終了したハンド履歴データ」の内容のみを事実として扱い、AI特有の勝手な勝率計算（ハルシネーション）は絶対に行わないでください。
